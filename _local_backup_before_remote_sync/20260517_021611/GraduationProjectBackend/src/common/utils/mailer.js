...
export async function sendNotificationEmail({ to, title, message, actionUrl }) {
  if (!smtpConfigured()) {
    if (env.nodeEnv !== "production") {
      console.log(`[DEV][EMAIL] Notification for ${to}: ${title} - ${message}`);
      return;
    }
    throw new Error("SMTP is not configured");
  }
  ...
  const safeMessage = escapeHtml(message || "You have a new ProjectHub notification.");
  const actionTarget =
    actionUrl && String(actionUrl).startsWith("/") && env.frontendUrl
      ? `${String(env.frontendUrl).replace(/\/+$/, "")}${actionUrl}`
      : actionUrl || env.frontendUrl || "";
  const safeActionUrl = escapeHtml(actionTarget);
  const brandName = escapeHtml(env.mailBrand ?? "GPMS");
  const hasAction = Boolean(actionUrl || env.frontendUrl);
    ...
    <div style="margin:0;padding:24px;background:#f8fafc;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;color:#0f172a;">
      <div style="max-width:560px;margin:0 auto;background:#ffffff;border:1px solid #e2e8f0;border-radius:18px;overflow:hidden;">
        <div style="padding:24px 28px;border-bottom:1px solid #e2e8f0;">
          <p style="margin:0;color:#2563eb;font-size:12px;font-weight:800;letter-spacing:.12em;text-transform:uppercase;">${brandName}</p>
          <h1 style="margin:8px 0 0;font-size:22px;line-height:30px;">${safeTitle}</h1>
        </div>
        <div style="padding:24px 28px;">
          <p style="margin:0;color:#475569;font-size:15px;line-height:24px;">${safeMessage}</p>
          ${
            hasAction
              ? `<p style="margin:24px 0 0;"><a href="${safeActionUrl}" target="_blank" style="display:inline-block;padding:12px 16px;border-radius:12px;background:#2563eb;color:#ffffff;text-decoration:none;font-weight:800;font-size:14px;">Open ProjectHub</a></p>`
              : ""
          }
        </div>
      </div>
  ...
  const text = [title || "New notification", "", message || "You have a new ProjectHub notification.", actionTarget ? `Open: ${actionTarget}` : ""]
    .filter(Boolean)
    .join("\n");

  await sendMailSafe({
    to,
    subject: title || "New ProjectHub notification",
    html,
    text,
  });
}
