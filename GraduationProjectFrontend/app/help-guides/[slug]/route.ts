...
function drawGuideSection(
  ...
  const tip = section.bullets[0] ?? section.heading
  const headingLines = wrapText(section.heading, fontBold, 11, width - 44)
  const tipLines = wrapText(tip, fontRegular, 8.5, width - 28)
  const firstBullets = section.bullets.slice(1, 3)
  ...
  y -= 6
  y = drawTextBlock(page, tip, x + 14, y, fontRegular, 8.5, COLORS.muted, width - 28, 12)
  y -= 6
